...
class Meneger:
    ...
    def send_application(self,conn):
        ...
        cursor.execute("SELECT messages FROM clients WHERE user_id = ?",(send_user_id,))
        ...
        if result:
            ...
            cursor.execute("UPDATE clients SET messages = ? WHERE user_id = ?",(json.dumps(messages),send_user_id))
            conn.commit()
